...
class _DoctorMessagesScreenState extends State<DoctorMessagesScreen> {
  ...
  Future<void> _loadChatPreviews() async {
    ...
    try {
      ...
      // For doctors, we need to get chats with patients
      final chatPreviews = await MessageService.getChatPreviews(authProvider.user!.id, isDoctor: true);
      setState(() {
  ...
  Widget _buildChatPreviewCard(ChatPreviewModel chat) {
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(
          chat.doctorName, // TODO: This should be patient name for doctors - update service to return patient name
        ...
        onTap: () {
          context.push('/doctor/chat/${chat.chatId}?patientName=${Uri.encodeComponent(chat.doctorName)}&patientId=${chat.doctorId}');
        },
      ),
    );
  }
